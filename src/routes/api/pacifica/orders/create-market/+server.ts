...
/** Proxy signed POST to Pacifica `POST /api/v1/orders/create_market`. */
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const url = new URL(
		...
		`${normalizeBase(locals.pacificaApiBaseUrl)}/`
	);
		...
		if (!res.ok) {
			const err =
				...
				typeof (parsed as { error: unknown }).error === 'string'
					...
					: `Pacifica create market HTTP ${res.status}`;
			return json({ success: false as const, error: err, details: parsed }, { status: 502 });
		}
		return json({ success: true as const, data: parsed });
	} catch (e) {
		const message = e instanceof Error ? e.message : 'Create market order request failed';
