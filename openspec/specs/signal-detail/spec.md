...
The signal detail view SHALL display token A and token B with their respective allocations, a candlestick chart for price context, an entry price, a textual description, and a primary control labeled for opening a position. The layout SHALL remain usable on narrow viewports (no required horizontal scrolling for the main card content).

#### Scenario: Required content is visible

- **WHEN** the signal detail page is shown for a valid slug
- **THEN** the user sees token A and B with their allocation percentages, a candlestick chart region, entry price, description, and an **Open position** (or equivalent) action
